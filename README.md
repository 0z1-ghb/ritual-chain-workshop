...
3. **Reveal Phase** (after deadline, before judging): participants call `revealAnswer()` with their original answer and salt. The contract verifies the hash matches the commitment, then pushes the plaintext into the `submissions` array.
...
| `web/src/components/SubmitAnswer.tsx` | Two phase UI: commit (before deadline) and reveal (after deadline) |
